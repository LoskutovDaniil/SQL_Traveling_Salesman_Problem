...
(
	...
	nodes.point1,
	nodes.point2,
	nodes.cost + travel.cost AS cost,
	travel.places_count + 1 AS places_count,
	travel.path || nodes.point1 AS path,
	nodes.point1 = ANY(travel.path) AS flag_false,
	travel.cost_array || nodes.cost AS cost_array
	FROM nodes
	JOIN travel ON nodes.point1 = travel.point2
	WHERE NOT travel.flag_false
)

SELECT DISTINCT cost - cost_array[5] AS total_cost,
	path AS tour
	FROM travel
...
SELECT *
FROM finish_roads
WHERE total_cost = (SELECT min(total_cost) FROM finish_roads)